...
# (id, dep_repo, path)
df_files = sc_sql.sql("""
    SELECT id, repo_name as dep_repo, path
    FROM files
    """)
...
# (dep_repo, name_set)
tmp2 = tmp1.select('dep_repo', 'path') \
    .map(lambda row: (row.dep_repo, row.path.split('/'))) \
    .aggregateByKey(set(), seqOp1, combOp1)

# (dep_repo, dep_set)
tmp3 = tmp1.select('dep_repo', 'dep') \
    .rdd \
    .aggregateByKey(set(), seqOp2, combOp2)

def remove_self_refs(tup):
    dep_repo = tup[0]
    dep_set, name_set = tup[1]
    dep_set.difference_update(name_set)
    return [(dep_repo, dep) for dep in dep_set]

# (dep_repo, filtered_dep)
rdd_query_input = tmp3.join(tmp2).flatMap(remove_self_refs)
...
def match_repos(tups):
    ...
    for mb in minibatch(tups, 10):
        ...
        for tup in mb:
            dep = tup[1]
            query['query']['constant_score']['filter']['term']['repo_name'] = dep
            body_elems.append('{}')
        ...
        for tup, response in zip(mb, responses):
            ...
            if len(hits) != 0:
                dst_repo = hits[0]['_source']['repo_name']
                dep_repo = tup[0]
                _, src_repo = dep_repo.split('/')
                # Filter out forks
...
topKrepos = rdd_repo_graph.top(K, key=lambda tup: tup[-1])

# Briefly visualize the top 10 repos
for tup in islice(topKrepos, 0, 10):
...
riak_buckets_top[0].new('K', K).store()

for k, tup, riak_bucket in zip(range(len(topKrepos)),
                               topKrepos,
